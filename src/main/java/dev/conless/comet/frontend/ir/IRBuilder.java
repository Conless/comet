...
public class IRBuilder extends IRManager implements ASTVisitor<IRNode> {
  ...
  public IRNode visit(FuncDefNode node) throws BaseError {
    ...
    var funcNode = new IRFuncDefNode(node.getName(), paramNodes, new IRType(node.getReturnType(), Case.PARAM));
  ...
  public IRNode visit(MemberExprNode node) throws BaseError {
    ...
    if (!objectType.depth.equals(0)) {
      instList.setDest(new IRFunc("__array_size", object, GlobalScope.irIntType));
  ...
  public IRNode visit(ForStmtNode node) throws BaseError {
    ...
    instList.addNode(new IRCommentNode("for " + node.getInit().toString() + node.getCondition().toString() + ";" + node.getUpdate().toString()));
